...
import { IconApi, IconArrowUpRight, IconGrid, IconServer } from '@/components/icons'
...
export default function Projects() {
  ...
  return (
    <section id="projects" ref={sectionRef} className="site-shell section-border py-16 sm:py-20">
      <div className={cn('section-reveal', visible && 'section-reveal-visible')}>
        <header className="mb-10 flex flex-wrap items-end justify-between gap-4">
          ...
          <p className="text-[10px] uppercase tracking-[0.14em] text-signal-red/68 sm:text-[11px] sm:tracking-[0.2em]">
            ...
            return (
              <div key={project.id} className={cn('project-card-stage', visible && 'project-card-stage-visible')} style={stageStyle}>
                <Card
                  ...
                  className={cn(
                    'project-card group relative flex h-full flex-col overflow-hidden border-white/15 p-4 transition-all duration-300 ease-system-ease sm:p-6',
                    signalClass
                  )}
                >
                  ...
                  <div
                    className={cn(
                      'pointer-events-none absolute right-4 top-4 hidden max-w-[62%] truncate text-right text-[9px] tracking-[0.14em] opacity-0 transition-all duration-300 ease-system-ease group-hover:translate-x-0 group-hover:opacity-100 group-focus-within:opacity-100 sm:block',
                  ...
                  <RainText
                    ...
                    text={project.title}
                    play={visible}
                    className="font-space text-lg font-medium text-white transition-transform duration-300 group-hover:translate-x-0.5 sm:text-xl"
                    durationMs={700}
                    staggerMs={18}
                    startY={-54}
                    bounceY={8}
        ...
        {activeProject ? (
          <article className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr] xl:gap-7">
            ...
            <div className="space-y-5">
              <div className="group relative flex aspect-[16/10] items-center justify-center overflow-hidden border border-white/10 bg-[#050505]">
                ...
                {activeProjectImage ? (
                  <img
                    ...
                    loading="lazy"
                    decoding="async"
                    className="absolute inset-0 h-full w-full object-cover object-center opacity-65 grayscale saturate-0 contrast-110 transition-[filter,transform,opacity] duration-700 ease-system-ease group-hover:opacity-85 group-hover:grayscale-0 group-hover:saturate-100 group-hover:contrast-100 group-hover:scale-[1.02]"
                  />
                ) : (
                  <div className="relative z-10 flex items-center gap-2 text-white/35">
                    <IconGrid size={22} />
                    <span className="text-xs uppercase tracking-[0.14em] sm:tracking-[0.22em]">PROJECT_PREVIEW</span>
                  </div>
                )}
              </div>
            </div>
